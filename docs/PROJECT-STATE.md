...
2. T2 單幀管線（PR #45 `c7ee145`）已 merge。T3 有界 session 與兩策略已完成交付，並完成 D1 §11.3 continuity 位移界線初值（0.50）與命名測試。T5 同意加密 TTL 可恢復刪除已完成交付（ResearchRecorder＋ResearchKeyProvider，僅 synthetic 驗證）。T4 capture pump／controller 已完成交付（CaptureSource＋LiveController，latest-slot1，drop 端到端非零可達，S1 camera-free 重跑全 PASS）。下一步依 Lead 派工（T6 回放）。
