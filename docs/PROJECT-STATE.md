...
2. T2 單幀管線（PR #45 `c7ee145`）已 merge。T3 有界 session 與兩策略已完成交付，並完成 D1 §11.3 continuity 位移界線初值（0.50）與命名測試。T5 同意加密 TTL 可恢復刪除已完成交付（ResearchRecorder＋ResearchKeyProvider，僅 synthetic 驗證）。T4 capture pump／controller 已完成交付（CaptureSource＋LiveController，latest-slot1，drop 端到端非零可達，S1 camera-free 重跑全 PASS）。T6 回放 ground truth 隔離與報告已完成交付（replay_session＋summarize，含 T5 N1 dims cap，僅 synthetic 驗證）。T7 本機視窗與研究 CLI 已完成交付（DesktopSession headless bindings＋research CLI live/replay/delete＋runbook，含 T4 N2 雙模式併用例與 T6 N2 replayed 標記，僅 synthetic/camera-free 驗證，真機待 T8）。T8 研究啟用驗收已完成交付（T7 N1 真正 timeout 語義＋CLI lifecycle/故障矩陣＋runbook 收尾；真人 smoke：hardware-ready; participant-smoke blocked）。Phase 2A T1–T8 實作鏈完成，可宣稱工具就緒，不可宣稱校準完成/誤認率 0/準備部署。
