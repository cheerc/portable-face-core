...
成功定義分兩階段，互不替代（2026-09-15 修訂，decision `d-20260915071553820286-1`）：

**Phase 2A ＝ 工具本身是準的。** 驗收軸是前處理的正確性與不變性，加上本團隊可自足完成的 §9 項目。**2A 不設任何準確率目標**——設了就是拿門檻去擬合資料。這個階段回答的是「這把尺有沒有刻歪」。

**Phase 2B ＝ 用這把尺量出來的結果有沒有效。** §5 兩條對照臂、§8 session 級分母、development 與 sealed holdout 切分，以及未註冊參與者 session，全部屬於 2B。這個階段回答的是「這個策略在真實 session 上辨識度如何」。

分界理由：2B 的每一項都依賴參與者的個別同意，時程不在專案控制內；綁進 2A 會讓 2A 永遠沒有可達終點。且未註冊者的拒識行為本即 open-set 的研究問題，不是工具可不可用的問題。

零形變並不保證認得對——單張註冊照對現場外觀的覆蓋殘差是真實存在的（實測：零形變下探針 top-1 仍可能非本人）。那屬 2B，不得用來否定 2A 的達成，也不得反過來讓 2A 去承諾準確率。

Phase 2B 的完整研究契約（分母與結果定義、最小診斷、分析觸發、development／holdout 與改善關卡）見 [Phase 2B 研究規格](2026-09-16-phase2b-mac-recognition-research.md)。本設計的 §5／§8 仍是其上位邊界，兩者衝突時以本設計為準。
...
擬定研究 profile：每 session 最長 5 秒、相鄰推論取樣至少相隔 200ms、至多 26 幀（含 t=0 與 t=5000 兩端點）、最新幀優先、佇列上限 1；此為待 spike 驗證的工程預算，不是已選的辨識水位。第一幀 t=0 立即取樣，後續至少相隔 200ms，第 26 幀名目 t=5000ms；若仍取舊上限 25，最後一幀名目只到 t=4800ms，不能形成完整 5 秒窗口。跨欄位必要條件：`max_frames >= ceil(timeout_ms / sample_interval_ms) + 1`；若未來 profile 不滿足，完整 deadline window 不可達，必須 fail closed／拒絕開始（具體 production 實作另待 plan＋operator go，Issue #84 保持 open 作 production follow-up）。profile 必須存入每次結果，修改後產生新版本；禁止依單次結果自動延長 session。（decision `d-20260920132145277296-1`）
...
2. **研究抽樣影像**：獨立明確同意後才啟用。每次 session UI 顯示保存狀態；僅保存上述取樣幀（上限 26 張、5 秒，含 t=0 與 t=5000 兩端點；decision `d-20260920132145277296-1`），不錄音、不連續背景錄影。預設 TTL 7 天。為重跑 detector，需要有限全幀；這是研究 recorder 的明列例外，不是 Face Core exemplar 政策變更。使用單人受控背景；觀察到多人即停止保存並丟棄本次尚未提交的影像 bundle。不得宣稱偵測器能發現所有背景旁人，仍須實驗場地控制。
...
- 真機人工 smoke（**2A 範圍**）：**一位已註冊參與者**；測試程式輸出與 UI 對上、保存開關明顯、可刪除／回放。不設「必須認對」來掩蓋模型限制。
- 引導 UI 為 **pyside6** 真窗（Cocoa 抉擇已關閉），須提供**方形人頭對齊框**，使採集影像為方形且不受相機直拍／橫拍影響。此為採集側一致性措施，**不替代**母規格 §6 的長寬比不變性要求；兩層都必須成立。
- **前處理不變性（2A 新增門）**：同一來源影像在不同長寬比封裝下，還原後的框／landmark 幾何與 embedding 須在容差內一致；非正方輸入的 Layer-A fixture 必須通過。
- **一位已同意未註冊參與者的 session 移至 Phase 2B**（見 §1），其依賴第二位參與者的個別同意，**不構成 2A 的收尾條件**。
